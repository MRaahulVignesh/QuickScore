...
class GraderCohere:
    def __init__(self, cohere_api_key, weaviate_api_key, weaviate_url, class_name, no_of_k):
        ...
        if class_name is not None:
            weaviate = Weaviate(self.client, self.class_name, "text").as_retriever(
                search_kwargs={"k": self.no_of_k}
            )
            self.chain = RetrievalQA.from_chain_type(
                llm=Cohere(cohere_api_key = cohere_api_key,temperature=0), retriever=weaviate
            )
        else:
            self.chain = cohere.Client(cohere_api_key)

    def grade(self, list_json, class_name):
